...
# Βάζουμε key για να μην χάνεται η αναφορά
...
# 1. Προσπάθεια ανάγνωσης Cookie
cookie_name = "cu_user_final" # Νέο όνομα για καθαρή αρχή
cookie_user = cookie_manager.get(cookie=cookie_name)

# 2. LOGIC: Αν ΔΕΝ είμαστε authenticated, ψάχνουμε το cookie
if not st.session_state.authenticated:
    if cookie_user:
        # Α. Βρήκαμε Cookie -> Login
        if cookie_user in SYSTEM_USERS:
            st.session_state.authenticated = True
            st.session_state.system_username = cookie_user
            st.rerun()
    else:
        # Β. Δεν βρήκαμε Cookie -> Μήπως αργεί να φορτώσει;
        # Ελέγχουμε αν είναι η "πρώτη προσπάθεια" μετά το refresh
        if "retry_cookie" not in st.session_state:
            st.session_state.retry_cookie = True
            time.sleep(1) # Περιμένουμε 1 δευτερόλεπτο
            st.rerun() # Ξανατρέχουμε τον κώδικα
        else:
            # Αν έχουμε ήδη περιμένει και πάλι δεν υπάρχει, τότε όντως δεν υπάρχει
            pass 
...
def login_page():
    ...
    # --- ΦΑΣΗ 2: 2FA & Cookie Set ---
    else:
        with st.container(border=True):
            st.info(f"User: **{st.session_state.system_username}**")
            otp_code = st.text_input("Admin Code (6-digit)", max_chars=6)
            ...
            if st.button("Login 🚀", use_container_width=True, type="primary"):
                ...
                if totp.verify(otp_code):
                    ...
                    cookie_manager.set(cookie_name, st.session_state.system_username, expires_at=expires)
                    
                    st.success("Σύνδεση...")
                    time.sleep(1)
                    st.rerun()
                else:
                    st.error("Λάθος κωδικός 2FA")
    ...
    with st.container(border=True):
        ...
        if st.button(f"ΕΝΕΡΓΟΠΟΙΗΣΗ ({times}x) 🔥", use_container_width=True, type="primary"):
            ...
            for i in range(times):
                ...
                bar.progress((i+1)/times); time.sleep(0.2)
            st.success(f"Ολοκληρώθηκε: {succ}/{times}")
